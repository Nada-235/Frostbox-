...
const CHIP_STYLES = {
  fresh: 'bg-[#E4F5F2] text-mint-deep',
  soon: 'bg-[#FDF0DD] text-[#B97317]',
  expired: 'bg-[#FBE4E1] text-berry',
};
...
function ItemCard({ item, lang, onOpen }){
  ...
  return (
    ...
    >
      <div className="w-[52px] h-[52px] rounded-2xl shrink-0 bg-frost flex items-center justify-center text-2xl overflow-hidden border border-line relative">
    ...
    <div className="flex items-center justify-between pb-4">
      <div>
        <div className="text-[13px] text-fog font-medium flex items-center">
          <span className="w-2 h-2 rounded-full bg-mint inline-block me-1.5 shadow-[0_0_0_3px_rgba(79,182,168,0.2)]" />
      ...
      <button
        ...
        className="force-mono bg-kale text-card text-xs font-semibold tracking-wide px-3 py-1.5 rounded-full border-none cursor-pointer"
      >
        {state.code}
      </button>
    </div>
...
export function FridgeBody(){
  ...
  const banner = dueReminders.length > 0 && (
    <div
      ...
      style={{ background: 'linear-gradient(135deg, var(--color-berry), #C43F30)' }}
    >
  ...
  const filters = (
    <div className="w-full bg-[#EAF2F0] sticky z-[99] pb-3.5">
      <div className="flex bg-[#EAF2F0] rounded-xl p-[3px] mb-3.5 sticky top-0 z-40">
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1.5 mb-4 glass border-b border-line pt-2">
        ...
        {FOOD_CATEGORIES.map(c => (
          <CatChip key={c.id} selected={categoryFilter === c.id} onClick={() => dispatch({ type: 'SET_CATEGORY_FILTER', value: c.id })}>
  ...
  if(!totalCount){
    ...
    sections = FOOD_CATEGORIES.map(category => {
      ...
      return (
        <div key={category.id}>
          <SectionLabel icon={category.icon}>{catLabel(category, lang)}</SectionLabel>
          {inCategory.map(item => (
