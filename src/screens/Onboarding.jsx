...
export function Onboarding(){
  ...
  return (
    ...
    >
      ...
      <button
        onClick={handleStart}
        className="w-full max-w-[300px] py-4 px-5 rounded-2xl border-none text-base font-semibold cursor-pointer mb-3 shrink-0 active:scale-[0.97] transition-transform bg-kale text-card"
      >
        {t('start_fridge')}
      </button>
      <button
      ...
      {state.joinBoxOpen && (
        <div className="mt-2.5 w-full max-w-[300px]">
          ...
          <button
            onClick={handleJoin}
            disabled={joining}
            className="w-full py-4 px-5 rounded-2xl border-none text-base font-semibold cursor-pointer active:scale-[0.97] transition-transform bg-kale text-card disabled:opacity-60"
          >
